...
model = dict(
    ...
    backbone=dict(
        type='YuNetBackbone',
        stage_channels=[[3, 16, 16], [16, 64], [64, 64], [64, 64]],
        downsample_idx=[0, 2],
        out_idx=[3]),
    neck=dict(type='WWHead_TFPN', in_channels=[64], out_idx=[0]),
    bbox_head=dict(
        type='YuNet_Head',
        num_classes=1,
        in_channels=64,
        ...
        prior_generator=dict(type='MlvlPointGenerator', offset=0, strides=[8]),
        # loss_cls=dict(
